...
export default function DashboardPage() {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Onboarding: SDK selection (dummy instructions for now) */}
      {!hasData && (
        <OnboardingSDKSelector />
      )}
...
type OnboardingChannel = 'web' | 'mobile' | 'server';

function OnboardingSDKSelector() {
  const [channel, setChannel] = useState<OnboardingChannel>('web');

  const sdkGroups: Record<OnboardingChannel, { id: string; name: string; tag: string; language: string; dummy: boolean }[]> = {
    web: [
      { id: 'js-browser', name: 'JavaScript (Browser)', tag: 'Web', language: 'JavaScript', dummy: true },
      { id: 'nextjs', name: 'Next.js', tag: 'Web', language: 'TypeScript', dummy: true },
    ],
    mobile: [
      { id: 'ios-swift', name: 'iOS (Swift)', tag: 'Mobile', language: 'Swift', dummy: true },
      { id: 'android-kotlin', name: 'Android (Kotlin)', tag: 'Mobile', language: 'Kotlin', dummy: true },
    ],
    server: [
      { id: 'node-server', name: 'Node.js', tag: 'Server', language: 'JavaScript', dummy: true },
      { id: 'go-server', name: 'Go', tag: 'Server', language: 'Go', dummy: true },
    ],
  };

  const currentSdks = sdkGroups[channel];

  return (
    <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-[24px] p-6 space-y-6">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          <h2 className="text-xl font-semibold mb-1">Where do you want to send data from?</h2>
          <p className="text-sm text-gray-400">
            Choose the SDKs you plan to use. Instructions below are <span className="font-semibold text-gray-300">dummy examples</span> until official SDKs are published.
          </p>
        </div>
      </div>

      <div className="inline-flex bg-white/5 border border-white/10 rounded-[999px] p-1 gap-1">
        <button
          onClick={() => setChannel('web')}
          className={`px-4 py-1.5 rounded-[999px] text-sm font-medium cursor-pointer transition-all ${
            channel === 'web' ? 'bg-white text-black' : 'text-gray-400 hover:text-white'
          }`}
        >
          Web
        </button>
        <button
          ...
          className={`px-4 py-1.5 rounded-[999px] text-sm font-medium cursor-pointer transition-all ${
            channel === 'mobile' ? 'bg-white text-black' : 'text-gray-400 hover:text-white'
          }`}
        >
          Mobile
        </button>
        <button
          ...
          className={`px-4 py-1.5 rounded-[999px] text-sm font-medium cursor-pointer transition-all ${
            channel === 'server' ? 'bg-white text-black' : 'text-gray-400 hover:text-white'
          }`}
        >
          Server
        </button>
          ...
          >
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <div className="text-xs text-gray-500">{sdk.language} • {sdk.tag}</div>
              </div>
            ...
            <div className="rounded-xl bg-black/40 border border-white/10 p-3">
              <p className="text-[11px] text-gray-400 mb-2">
                SDK integration snippets here are placeholders. They show how you <em>will</em> send events to Norai using <code className="font-mono">/v1/events</code>.
              </p>
              <pre className="text-[11px] leading-relaxed text-gray-200 font-mono whitespace-pre-wrap">
{`// DUMMY SDK EXAMPLE – do not use in production yet
// Coming soon: official ${sdk.name} SDK for Norai

// Example event:
POST /v1/events
Content-Type: application/json
X-Norai-API-Key: <your_api_key>
